...
grow too large, and they will be filtered away. But our data bind only knows that
the array has gotten shorter, so it just exits the last circle tags, even if
those weren't the ones that should be removed. This issue is illustrated in the
sketch below,
